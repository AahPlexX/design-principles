...
export const performance: Principle = {
  ...
  mistakes: [
    ...
    { name: "Serving one image size to every screen", body: html("A single 1600px-wide file shipped to a 375px-wide phone forces that phone to download and decode four times more pixel data than it will ever render. <code>srcset</code> and <code>sizes</code> — or an image CDN (a content delivery network: a service that serves resized, optimized versions of an image from servers near the reader) that generates them automatically — let the browser choose the file that actually matches the screen.") },
  ...
  checklist: [
    ...
    html("Custom fonts use <code>font-display: swap</code>, or are preloaded with <code>&lt;link rel=\"preload\"&gt;</code> — which tells the browser to fetch a critical resource like a font immediately instead of waiting to discover it later in the page — so text is never invisible while a font downloads."),
    ...
    html("Critical cross-origin resources — a font host, an image CDN — get a <code>&lt;link rel=\"preconnect\"&gt;</code>, so the browser opens that connection before it actually needs to fetch anything from it."),
  ],
  practiceCourseId: null,
  goDeeper: [
    { lead: "Core Web Vitals", body: html("Google's three headline metrics turn the ideas above into precise, testable numbers. Largest Contentful Paint (how fast the biggest visible element appears) is \"good\" under 2.5 seconds, \"poor\" past 4. Cumulative Layout Shift (how much content moves unexpectedly) is \"good\" under a score of 0.1, \"poor\" past 0.25. Interaction to Next Paint (how fast the page visibly responds after a click or tap) is \"good\" under 200 milliseconds, \"poor\" past 500. Those particular numbers aren't arbitrary: they echo the <strong>Doherty Threshold</strong>, a 1982 finding that a system responding in under roughly 400 milliseconds keeps a person's attention and sense of flow, while anything slower lets it drift — INP's \"good\" and \"poor\" bars sit just under and just over that same line rather than at some round number. Google scores a page at the 75th percentile of real visits, meaning three out of four real page loads need to land in \"good\" — not just your fastest test run. Interaction to Next Paint replaced an older metric called First Input Delay as the third Core Web Vital in 2024, because First Input Delay only measured a page's very first interaction, not the ones later in a session that actually cause frustration.") },
