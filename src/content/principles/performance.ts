...
export const performance: Principle = {
  ...
  checklist: [
    ...
    html("Custom fonts use <code>font-display: swap</code> (or are preloaded) so text is never invisible while a font downloads."),
  ...
  goDeeper: [
    { lead: "Core Web Vitals", body: html("Google's three headline metrics turn the ideas above into precise, testable numbers. Largest Contentful Paint (how fast the biggest visible element appears) is \"good\" under 2.5 seconds, \"poor\" past 4. Cumulative Layout Shift (how much content moves unexpectedly) is \"good\" under a score of 0.1, \"poor\" past 0.25. Interaction to Next Paint (how fast the page visibly responds after a click or tap) is \"good\" under 200 milliseconds, \"poor\" past 500. Google scores a page at the 75th percentile of real visits, meaning three out of four real page loads need to land in \"good\" — not just your fastest test run. Interaction to Next Paint replaced an older metric called First Input Delay as the third Core Web Vital in 2024, because First Input Delay only measured a page's very first interaction, not the ones later in a session that actually cause frustration.") },
