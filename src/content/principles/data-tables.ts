...
export const dataTables: Principle = {
  ...
  checklist: [
    ...
    html("Sorting or filtering exists once a table has enough rows that scanning alone isn't practical, and any sort control is a real, keyboard-reachable button with <code>aria-sort</code> reflecting the active column."),
  ],
  ...
  goDeeper: [
    ...
    { lead: "Why WCAG allows this scroll instead of banning it", body: html("WCAG's reflow criterion generally requires content to avoid scrolling in two directions at once, but it explicitly exempts content — data tables among them — whose meaning genuinely depends on a two-dimensional grid. A table scoped to scroll within its own container, leaving the rest of the page reflowing normally, is the compliant pattern the exception was written for; a table that drags the entire page sideways is not.") },
  ],
