...
export const accessibility: Principle = {
  ...
  definition: html("Accessibility is designing a site so it still works for someone who can't use a mouse, can't see the screen, or can't see it well — using only a keyboard, a screen reader (software that reads the page aloud or sends it to a braille display), or magnification."),
  whyItMatters: [
  ...
  examples: [
    ...
    },
    {
      ...
      good: {
        ...
        note: html("The link is the very first focusable element on the page. It stays visually hidden until Tab moves focus onto it, then appears and jumps straight to <code>#main</code> — letting a keyboard user skip the repeated navigation on every page instead of tabbing through it each time (WCAG — the Web Content Accessibility Guidelines, the standard most accessibility laws and audits are measured against — 2.4.1, \"Bypass Blocks\"). <code>tabindex=\"-1\"</code> on <code>&lt;main&gt;</code> matters as much as the link itself: a <code>&lt;main&gt;</code> isn't natively focusable, so without it, activating the link only scrolls the page — focus stays on the link, and the next Tab press continues from there instead of from the content it just jumped to."),
      },
      bad: {
