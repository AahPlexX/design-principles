...
export const contentMicrocopy: Principle = {
  ...
  goDeeper: [
    ...
    { lead: "Checking readability without guessing", body: html("A Flesch reading-ease score — a formula that scores English text from 0 to 100 using average sentence length and average syllables per word, where a higher number means easier to read — gives a concrete number instead of a gut feeling about whether copy is simple enough. It's calibrated specifically for English; it doesn't carry over to a translation, and other languages need their own formula rather than the same number meaning the same thing. Without any tooling, and in any language, reading the copy aloud and noting exactly where you stumble or run out of breath catches the same class of problems — overly long sentences, awkward phrasing — that a formula would flag.") },
  ],
