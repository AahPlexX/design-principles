...
export const internationalizationLocalization: Principle = {
  ...
  whyItMatters: [
    ...
    html("None of this is a rounding-error audience either. Right-to-left languages aren't an edge case: Arabic alone has hundreds of millions of native speakers, and Hebrew, Persian, and Urdu add tens of millions more — an interface that only works left-to-right isn't unpolished for a market that size, it's unusable. The smaller-looking failures cost real money too: a date field that prints \"03/04/2026\" reads as March 4th in the United States and the 3rd of April almost everywhere else, and a shipment, an appointment, or a contract built on that misreading doesn't look like a translation problem when it goes wrong — it looks like the product got the order wrong."),
  ],
  coreRule: [
  ...
  examples: [
    ...
    },
    {
      ...
      good: {
        ...
        note: html("The button grows or shrinks with whatever text ends up inside it, so a translated label that's longer or shorter than the English original is never clipped — <code>max-width: 100%</code> stops the button from overflowing its container, and a label too long to fit on one line wraps onto a second instead of being cut off."),
      },
      bad: {
    ...
    },
    {
      ...
      good: {
        ...
        note: html("The browser's built-in <code>Intl</code> API looks up the reader's actual locale and spells the date out the way that locale expects — no month/day guessing required, for the reader or for you. It also accepts a <code>calendar</code> option (<code>islamic</code> for the Hijri calendar, <code>hebrew</code> for the Hebrew calendar, and others) for locales that don't use the Gregorian calendar at all."),
      },
      bad: {
  ...
  mistakes: [
    ...
    { name: "Hardcoding date, number, or currency formats", body: html("Concatenating month, day, and year by hand, or prepending a hardcoded \"$\", locks the output to one region's convention — and it's not just symbols and day order: whether an amount reads <code>€1,234.56</code> or <code>1.234,56 €</code> depends on the same kind of convention, since symbol placement (before or after the amount) and decimal-separator style (a period or a comma) both vary by locale. A locale-aware formatting API — <code>Intl.DateTimeFormat</code> for dates, <code>Intl.NumberFormat</code> with <code>style: \"currency\"</code> for money — produces the right separators, symbol placement, and date order for whoever's actually reading it. The same logic covers units: a distance, weight, or temperature value needs the reader's expected unit system (metric or imperial) as much as a date needs the reader's expected day/month order.") },
  ...
  goDeeper: [
    ...
    { lead: "The bidi algorithm handles inline mixing, not layout", body: html("The Unicode Bidirectional Algorithm usually keeps embedded left-to-right content — a product code, an English brand name — reading in the correct order inside a right-to-left sentence with no extra markup. It governs the order of characters and words inside a run of text and nothing more; it does nothing for a hardcoded <code>padding-left</code> or a toolbar's icon order, which is why layout still needs logical properties and mirrored icons on top of it. It also needs help for text whose direction it can't infer on its own — dynamically inserted content like a username or a search query, sitting next to punctuation, can come out in the wrong order without <code>&lt;bdi&gt;</code> or <code>dir=\"auto\"</code> to isolate it from the surrounding text.") },
