...
export const emptyErrorStates: Principle = {
  ...
  whyItMatters: [
    ...
    html("This moment — the search that turns up nothing, the brand-new account with an empty dashboard, the checkout that fails on the last step — is not a rare corner of a product. A checkout failure does outsized damage precisely because of when it lands: the <strong>Peak-End Rule</strong> holds that people judge an experience mostly by its single most intense moment and by how it ends, not by averaging everything that came before it, so an error on the very last step becomes the ending a shopper remembers, outweighing every step that went smoothly up to that point. For a meaningful share of visitors, one of these is the very first real screen they see, before the \"actual\" product has had any chance to prove itself. A first-time visitor who mistypes a search, or opens an account before adding anything to it, forms an opinion of the whole product from that one blank or broken screen, not from the polished pages three clicks deeper that they may never reach."),
  ...
  examples: [
    ...
    },
    {
      ...
      good: {
        ...
        note: html("The form's <code>role=\"search\"</code> marks it as a search landmark rather than a generic form, so assistive tech can jump straight to it instead of treating it as just another form on the page. Names what likely happened, then offers two different routes forward — home, or a search for whatever the reader was actually looking for — instead of leaving them stranded on a dead link."),
      },
      bad: {
    ...
    },
    {
      ...
      good: {
        ...
        note: html("<code>aria-invalid=\"true\"</code> flags the field itself as currently failing validation, and <code>aria-describedby=\"email-err\"</code> ties the error message to that field so a screen reader announces both together instead of just the input's label on its own. Points at the exact field, states what's wrong with it, and shows what a correct value looks like, so the reader can fix it in one pass instead of re-reading the whole form."),
      },
      bad: {
  ...
  mistakes: [
    ...
    { name: "Error states with no way forward", body: html("A failed page load with no retry button, no link back, and no contact option leaves the reader stuck exactly where the error occurred. A retry action itself needs a mechanism to be worth adding: it should show its own pending state while it's retrying, so a second click doesn't fire a duplicate request, and it should offer a different next step — a link back, a contact option — once it's failed enough times, rather than inviting the reader to click retry forever. The 404 example above shows how little it takes to turn a dead end back into forward progress.") },
  ...
  checklist: [
    ...
    html("A transient failure that doesn't stop the reader from continuing what they were doing surfaces as a toast or snackbar (a small message that appears briefly near a screen edge without blocking the rest of the page), not the same persistent, page-occupying treatment used for an error that blocks the task entirely — but if it carries an action, like \"Couldn't save — retry?\", it stays on screen until the reader dismisses it or the retry succeeds, rather than auto-dismissing and taking the only way to recover along with it. Auto-dismiss is for a transient message with nothing to act on, like a plain \"Saved\" confirmation. Either way, the toast itself needs a <code>role=\"status\"</code> (or <code>aria-live=\"polite\"</code>) region so a screen reader announces it without the reader needing to be focused on it when it appears."),
  ],
  ...
  goDeeper: [
    ...
    { lead: "A first-run empty state is also an onboarding moment", body: html("See <a href=\"/design-principles/principles/onboarding-progressive-disclosure.html\">Onboarding &amp; Progressive Disclosure</a> — the brand-new-list example above, where nothing has been created yet, is the reader's first real screen in the product. What it shows and what it asks them to do first is as much an onboarding decision as an empty-state one.") },
  ],
