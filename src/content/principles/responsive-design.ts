...
export const responsiveDesign: Principle = {
  ...
  coreRule: [
    html("Design the layout to respond to the <em>content</em> running out of room, not to a fixed list of device widths. A <strong>breakpoint</strong> — the point where a layout switches to a different arrangement — belongs wherever your specific content starts to look cramped or oddly sparse, not at a guessed number like \"iPhone width\" or \"iPad width.\" Reach for flexible units (percentages, <code>fr</code>, <code>ch</code>, <code>rem</code>, <code>clamp()</code>) and let CSS Grid or Flexbox reflow the content first; add a breakpoint only once flexible units alone can't hold the layout together anymore."),
  ],
  examples: [
    {
    ...
    },
    {
      ...
      good: {
        ...
        note: html("Tells the phone's browser to lay the page out at its actual screen width, so the flexible CSS underneath it can take effect at all."),
      },
      bad: {
  ...
  mistakes: [
    ...
    { name: "Touch targets sized for a mouse pointer", body: html("A link or icon button under WCAG 2.2's 24×24px minimum (Success Criterion 2.5.8) is hard to tap accurately with a finger, even though it's effortless to click with a precise mouse cursor. 44×44px — the higher, AAA-level target size (Success Criterion 2.5.5) — is a more comfortable size to design to; treat 24×24px as the legal floor, not the goal.") },
