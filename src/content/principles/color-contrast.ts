...
export const colorContrast: Principle = {
  ...
  mistakes: [
    ...
    { name: "Relying on color alone to carry meaning", body: html("Red and green to mean error and success fails for colorblind readers if there's no icon, label, or shape backing it up — the color is a bonus signal, never the only one. Red-green confusion (protanopia and deuteranopia, the two most common forms) is the classic failure case because the eye's red- and green-detecting cone cells have overlapping, not distinct, sensitivity to wavelengths of light — a color that reads clearly as red to a typical viewer can read as muddy or greenish to someone with one of these conditions. Blue-yellow confusion (tritanopia) is rarer, but the fix is identical either way: never let color be the only signal.") },
