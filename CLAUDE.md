...
4. **Good vs. bad example(s)** — at least two scenarios (`examples`, plural), each a genuinely different real context the rule shows up in, not the same context restated. One worked example proves the rule holds once; several prove it's actually a rule. Each scenario gets a short context label (e.g. "Text over a photo", "A disabled-looking button") so a reader can jump to the one closest to what they're building.
   ...
   An example side (`good`/`bad`) can carry an optional **live preview** (`ExampleSide.preview`, `src/content/types.ts`) — a self-contained HTML document (own `<style>`, real demo markup, starting with `<!doctype html>`) rendered live in a sandboxed iframe next to the code sample. Reach for it when the difference is genuinely visual — a spacing value, a wrapped line, a contrast pair, a heading stack: reading `line-height: 1` in a code block and *seeing* two lines of text touch are not the same lesson, and the worked-example effect (Sweller's cognitive load research: novices learn better from a concrete demonstration than from prose describing one) is stronger the more concrete the demonstration is. Skip it for a side whose difference isn't visible on screen (a JS behavior, an ARIA attribute, a data-format rule) — the code sample alone is the right and sufficient teaching tool there, and an empty or decorative preview is worse than none. `scripts/gates/content-invariants.ts` enforces that a declared preview is non-empty and self-contained; nothing requires every example to have one.
5. **Common mistakes** — 3 or more specific, named failure modes, not generic warnings. Size to what the topic actually supports — a narrow principle may only have three real ones; don't invent a fourth to hit a round number, and don't stop at a round number if a rich topic genuinely has more. There is no upper cap — the old 3–6 ceiling was itself a formulaic constraint that forced real, distinct mistakes to be merged together just to fit a number CLAUDE.md never actually asked for, and it has been removed.
